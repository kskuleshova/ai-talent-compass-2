...
import { getVacancy, updateVacancy } from "@/lib/vacancies.functions";
import { ArrowLeft, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { Checkbox } from "@/components/ui/checkbox";

export const Route = createFileRoute("/_authenticated/vacancies/$id/edit")({
  head: () => ({ meta: [{ title: "Редагувати вакансію — Hirelens" }] }),
...
function EditVacancy() {
  ...
  const updateFn = useServerFn(updateVacancy);
  const { data, isLoading } = useQuery({ queryKey: ["vacancy", id], queryFn: () => fn({ data: { id } }) });
  ...
  // Pre-fill form once data has loaded
  useEffect(() => {
    if (data?.vacancy && !initialized) {
      const v = data.vacancy;
      setForm({
        title: v.title ?? "",
        job_description: v.job_description ?? "",
        hiring_manager_brief: v.hiring_manager_brief ?? "",
        must_have: v.must_have ?? "",
        nice_to_have: v.nice_to_have ?? "",
        screening_questions: v.screening_questions ?? "",
        test_task: v.test_task ?? "",
        historical_feedback: v.historical_feedback ?? "",
      });
      setHasTestTask(!!v.test_task?.trim());
      setHasHistoricalFeedback(!!v.historical_feedback?.trim());
      setInitialized(true);
    }
  }, [data, initialized]);
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      <Link to="/vacancies/$id" params={{ id }} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> До вакансії
      </Link>
      <h1 className="mt-4 text-2xl font-semibold tracking-tight">Редагувати вакансію</h1>
      <p className="mt-1 text-sm text-muted-foreground">Зміни вплинуть на майбутній AI-аналіз нових кандидатів.</p>
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Link to="/vacancies/$id" params={{ id }} className="rounded-md px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-accent">Скасувати</Link>
          <button
