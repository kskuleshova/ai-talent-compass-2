...
function CandidatePage() {
  ...
  const noteMut = useMutation({
    mutationFn: () =>
      noteFn({ data: { candidate_id: id, body: noteBody.trim() } }),
    onSuccess: async () => {
      setNoteBody("");
  ...
  // --------------------------
  // EXTRA ANALYSIS MUTATION
  // --------------------------
  const extraMut = useMutation({
    mutationFn: () =>
      updateFn({ data: { candidate_id: id, extra: extra.trim() } }),
    onSuccess: async () => {
      setExtra("");
      await qc.invalidateQueries({ queryKey: ["candidate", id] });
      await qc.refetchQueries({ queryKey: ["candidate", id] });
  ...
  const percent: number | undefined =
    analysis?.summary?.overall_match_percent;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {analysis ? (
                <>Проаналізовано {new Date(analysis.created_at).toLocaleString("uk-UA")}</>
              ) : (
                "Аналізу ще немає"
              )}
            </p>
              ...
              <div className="mt-5 space-y-6">
                ...
                <div>
                  ...
                  <dl className="grid grid-cols-2 gap-3 rounded-lg border border-border bg-background p-4 text-sm">
                    <SummaryItem
                      label="Поточна роль"
                      value={analysis.summary?.current_role}
                    />
                    <SummaryItem
                      label="Досвід"
                      value={analysis.summary?.years_of_experience}
                    />
                    <SummaryItem
                      label="Індустрії"
                      value={(analysis.summary?.industries ?? []).join(", ")}
                    />
                    <SummaryItem
                      label="Мови"
                      value={(analysis.summary?.languages ?? []).join(", ")}
                    />
                    <SummaryItem
                      label="Лідерський досвід"
                      value={analysis.summary?.leadership_experience}
                      full
                    />
                  </dl>
                </div>
                ...
                {analysis.suggested_questions?.length > 0 && (
                  <div>
                    <h3 className="mb-2 text-sm font-semibold">
                      Рекомендовані скринінгові питання
                    </h3>
                    <ol className="list-decimal space-y-2 pl-5 text-sm">
                      {analysis.suggested_questions.map(
                        (q: string, i: number) => (
                          <li key={i}>{q}</li>
                        )
                      )}
